...
def get_lenet():
    ...
    net = mx.sym.Convolution(source, kernel=(3, 3), pad=(1,1), stride=(1,1), num_filter=128)
    net = mx.sym.BatchNorm(net, fix_gamma=True)
    net = mx.sym.Activation(net, act_type="relu")
    net = mx.sym.Convolution(net, kernel=(3, 3), pad=(1,1), stride=(1,1), num_filter=128)
    net = mx.sym.BatchNorm(net, fix_gamma=True)
    net = mx.sym.Activation(net, act_type="relu")
    ...
    net = mx.sym.Activation(net, act_type="relu")
    net = mx.sym.Pooling(net, pool_type="max", kernel=(2,2), stride=(2,2))
    net = mx.sym.Convolution(source, kernel=(3, 3), pad=(1,1), stride=(1,1), num_filter=256)
    net = mx.sym.BatchNorm(net, fix_gamma=True)
    net = mx.sym.Activation(net, act_type="relu")
    net = mx.sym.Convolution(net, kernel=(3, 3), pad=(1,1), stride=(1,1), num_filter=256)
    net = mx.sym.BatchNorm(net, fix_gamma=True)
    net = mx.sym.Activation(net, act_type="relu")
    net = mx.sym.Convolution(net, kernel=(3, 3), pad=(1,1), stride=(1,1), num_filter=256)
    net = mx.sym.BatchNorm(net, fix_gamma=True)
    net = mx.sym.Activation(net, act_type="relu")
    net = mx.sym.Pooling(net, pool_type="max", kernel=(2,2), stride=(2,2))
    net = mx.sym.Convolution(source, kernel=(3, 3), pad=(1,1), stride=(1,1), num_filter=512)
    net = mx.sym.BatchNorm(net, fix_gamma=True)
    net = mx.sym.Activation(net, act_type="relu")
    net = mx.sym.Convolution(net, kernel=(3, 3), pad=(1,1), stride=(1,1), num_filter=512)
    net = mx.sym.BatchNorm(net, fix_gamma=True)
    net = mx.sym.Activation(net, act_type="relu")
    net = mx.sym.Convolution(net, kernel=(3, 3), pad=(1,1), stride=(1,1), num_filter=512)
    net = mx.sym.BatchNorm(net, fix_gamma=True)
    ...
    fc2 = mx.symbol.FullyConnected(data=fc1, num_hidden=1024)
    fc2 = mx.sym.Activation(fc1, act_type="relu")
    fc2 = mx.symbol.Dropout(fc1)
    fc3 = mx.symbol.FullyConnected(data=fc1, num_hidden=1024)
    fc3 = mx.sym.Activation(fc1, act_type="relu")
    fc3 = mx.symbol.Dropout(fc1)
    ...
    return mx.symbol.LinearRegressionOutput(data=fc3,name='softmax')

...
def get_lenet():
    ...
    net = mx.sym.Convolution(source, kernel=(5, 5), num_filter=20)
    net = mx.sym.BatchNorm(net, fix_gamma=True)
    net = mx.sym.Activation(net, act_type="relu")
    net = mx.sym.Pooling(net, pool_type="max", kernel=(2,2), stride=(2,2))
    net = mx.sym.Convolution(net, kernel=(3, 3), num_filter=20)
    net = mx.sym.BatchNorm(net, fix_gamma=True)
    net = mx.sym.Activation(net, act_type="relu")
    net = mx.sym.Pooling(net, pool_type="max", kernel=(2,2), stride=(2,2))
    net = mx.sym.Convolution(net, kernel=(3, 3), num_filter=20)
...
batch_size = 32
devs = [mx.gpu(0)]
data_train = mx.io.CSVIter(data_csv="./local_train-128x128-data.csv", data_shape=(30, 128, 128),
                           label_csv="./train-systole.csv", label_shape=(600,),
                           batch_size=batch_size)

data_test = mx.io.CSVIter(data_csv="./local_test-128x128-data.csv", data_shape=(30, 128, 128), label_csv="./train-systole.csv", label_shape=(600,), batch_size=batch_size) 

data_validate = mx.io.CSVIter(data_csv="./validate-128x128-data.csv", data_shape=(30, 128, 128),
                              batch_size=1)
...
data_test = mx.io.CSVIter(data_csv="./local_test-128x128-data.csv", data_shape=(30, 128, 128), label_csv="./train-diastole.csv", label_shape=(600,), batch_size=batch_size) 
...
systole_result = accumulate_result("./validate-label-128.csv", systole_prob)
diastole_result = accumulate_result("./validate-label-128.csv", diastole_prob)

...
random.seed(10)
test_frames  = get_frames("../../data/test")
print('read in test frames')

# Write the corresponding label information of each frame into file.

write_label_csv("test-label.csv", test_frames, None)
...
valid_lst = write_data_csv("test-64x64-data.csv", test_frames, lambda x: crop_resize(x, 64))

# Generate local train/test split, which you could use to tune your model locally.
train_index = np.loadtxt("test-label.csv", delimiter=",")[:,0].astype("int")
train_set, test_set = local_split(train_index)

split_to_train = [x in train_set for x in train_index]
split_csv("test-label.csv", split_to_train, "test_local_train-label.csv", "test_local_test-label.csv")
split_csv("test-64x64-data.csv", split_to_train, "test_local_train-64x64-data.csv", "test_local_test-64x64-data.csv")
